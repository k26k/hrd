...
public interface MemberRepository extends JpaRepository<Member, String> {
	...
	public boolean memberDisable(@Param("userId") String userId);
	
	@Transactional
	@Modifying
	@Query("UPDATE Member m SET m.enabled = 1 WHERE m.userId = :userId")
	public boolean memberEnable(@Param("userId") String userId);

	@Transactional
	@Modifying
	@Query("UPDATE Member m SET m.role = :role WHERE m.userId = :userId")
	public boolean changeRole(@Param("userId") String userId, @Param("role") Role role);
